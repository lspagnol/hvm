...
apt-get -y remove --purge mdadm lvm2 liblvm2app2.2 open-iscsi
apt-get -y remove --purge lxd lxd-client lxcfs lxc-common liblxc1
apt-get -y autoremove --purge
rm -rf /var/lib/lxd/
apt-get -y upgrade
apt-get -y upgrade linux-generic
apt-get -y install zfs joe htop arping mbuffer liblz4-tool smartmontools s-nail git
echo
...
for s in dbus rsyslog cron atd ; do service $s stop ; done
...
########################################################################
echo "* Installer les paquets de virutalisation"
...
- Copier le modèle de fichier de configuration local
  'cp /usr/local/hvm/etc/local.conf.dist /usr/local/hvm/etc/local.conf'
